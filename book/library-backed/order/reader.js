...
// 学生更改信息接口
router.post('/readerchange',(req,res) =>{
	let data = req.body;
	...
	conn.query(`update reader set readerName='${data.userName}',password='${userRegisterPwd}' 
...
router.post('/register', (req, res) => {
    ...
    conn.query(`select * from reader where phone='${data.phone}'`, (err, rs) => {
      ...
      if (rs.length > 0) {
        ...
        conn.query(`insert into reader values('${readerId}','${data.userName}','${userRegisterPwd}','${data.phone}',0,0,0,1)`, (err, rs) => {
          if (err) throw err;
          res.json({
            msg: '注册成功',
            status: 200
...
router.post('/continueborrow',(req,res)=>{
	...
	console.log('续借:',data);
		//更新借阅表状态
	conn.query(`update borrow set status='未还',borrowDate=now(),returnDate=date_add(NOW(), interval 1 month) 
