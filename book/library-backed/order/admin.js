...
router.post('/borrowslist', (req, res) => {
	conn.query(`select * from borrow`, (err, rs) => {
		let data = rs || []
		if (data.length == 0)
			res.json({
				msg: '管理员请求借阅记录为空',
...
router.post('/getbook', (req, res) => {
	let data = req.body
	conn.query(`select bookName from book where bookId ='${data.bookId}'`,(err,rs)=>{
		if (err) throw err;
		console.log("重新获取图书信息：",rs);
		if (rs.length > 0) {
		  res.json({
		    msg: '获取图书成功！',
			bookName:rs[0].bookName,
		    status: 200,		    
		  })
		} else {
		  res.json({
...
router.post('/adminchange',(req,res) =>{
	...
	conn.query(`update admin set userName='${data.userName}',password='${data.password}' 
	where id='${data.AdminId}'`, (err, rs) => {
	  if (err) throw err;
	  res.json({
	    msg: '更改成功',
	    status: 200
...
router.post('/adminaddbooks', (req, res) => {
	...
	conn.query(`select * from book where position = '${data.position}'`, (err, rs) => {
		if (rs.length > 0) {
			...
			conn.query(`select * from book where bookName = '${data.bookName}'` , (err,rs) =>{
				if(rs.length >0){
					res.send({
						msg: '已有该书籍存放！',
						status: 0
...
router.post('/delperson', (req, res) => {
	let data = req.body
	conn.query(`update reader set status=0 where readerId='${data.readerId}'`)
	res.send({
		msg: '删除读者成功',
		status: 200
	})
